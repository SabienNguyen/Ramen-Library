  ...
  return (
    <BaseSlider.Root data-slot="slider" className={cn('w-full', className)} {...props}>
      <BaseSlider.Control className="relative flex w-full touch-none items-center py-2 select-none">
        <BaseSlider.Track className="relative h-1.5 w-full grow overflow-hidden rounded-full bg-secondary">
          <BaseSlider.Indicator className="absolute h-full rounded-full bg-primary" />
        </BaseSlider.Track>
        <BaseSlider.Thumb aria-label={label} className="absolute top-1/2 block size-4 rounded-full border border-input bg-card shadow-card outline-none focus-visible:ring-2 focus-visible:ring-ring/40" />
      </BaseSlider.Control>
    </BaseSlider.Root>
  )
}
